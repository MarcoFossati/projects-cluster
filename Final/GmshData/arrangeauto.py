...
with open(
        r"C:\Users\Andrew\Documents\University\4th Year\Individual Project\PycharmProjects\Atmos_Models\twp_edit.lsn",
        "r") as searchfile:
...
def right_node_arranger(tan_lat,temp_column):
    ...
    output = list()
    for i in range(588,613,1):
        temp = float(data[i][temp_column])#*float(data[i][18])*100
        mini = list(fixed[i-588])
        mini.append(temp)
        output.append(mini)
