def datacall(alt,hem,lat,mnt):
    ...
    temp_min = min_temp_min + ((max_temp_min - min_temp_min)*(alt - float(data[templ_min][0])))/(float(data[templ_max][0]) - float(data[templ_min][0]))
    temp_max = min_temp_max + ((max_temp_max - min_temp_max)*(alt - float(data[templ_min][0])))/(float(data[templ_max][0]) - float(data[templ_min][0]))
    temp = temp_min + ((temp_max - temp_min)*(latl - min_col))/(max_col - min_col)

    wind_min = min_wind_min + ((max_wind_min - min_wind_min)*(alt - float(data[windl_min][0])))/(float(data[windl_max][0]) - float(data[windl_min][0]))
    wind_max = min_wind_max + ((max_wind_max - min_wind_max)*(alt - float(data[windl_min][0])))/(float(data[windl_max][0]) - float(data[windl_min][0]))
    ...
    if alt >= 20:
        ...
        press_min = min_press_min + ((max_press_min - min_press_min)*(alt - float(data[pressl_min][0])))/(float(data[pressl_max][0]) - float(data[pressl_min][0]))
        press_max = min_press_max + ((max_press_max - min_press_max)*(alt - float(data[pressl_min][0])))/(float(data[pressl_max][0]) - float(data[pressl_min][0]))
    ...
    print "Altitude:",alt,"km","\nLatitude:",lat,mag,'\n','-'*25,"\nTemperature:",round(temp,2),"K","\nPressure:",press,"Pa"\
        "\nDensity:",dens,"kg/m3","\nZonal Wind Speed:",round(wind,2),"m/s\n",'-'*25,'\nCOSPAR INTERNATIONAL REFERENCE ATMOSPHERE, 1986 (Monthly)'